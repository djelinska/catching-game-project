...
const addComment = async (req, res) => {
	try {
		...
		logger.info(`Dodanie komentarza pod profilem ${user.username}`, {
...
const deleteComment = async (req, res) => {
	try {
		...
		const user = await User.findOne({ _id: existingComment.profile_user });
		const loggedUser = await User.findById(req.user._id);

		await Comment.deleteOne({ _id: commentId });

		logger.info(`Usunięcie komentarza pod profilem ${user.username}`, {
			additionalInfo: loggedUser.username,
		});
